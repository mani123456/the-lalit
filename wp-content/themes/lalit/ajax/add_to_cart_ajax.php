	    ...
	    if($shippable_flag){
		    ...
		    if($passed_validation && WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation)){
			    ...
			    if(isMobile()){
			  	...
			  	$return_msg['text'] = 'There ';
			  	$return_msg['text'] .= (WC()->cart->cart_contents_count > 1) ? 'are' : 'is';
			  	$return_msg['text'] .= ' '.WC()->cart->cart_contents_count;
				$return_msg['text'] .= ' item';
				$return_msg['text'] .= (WC()->cart->cart_contents_count > 1) ? 's' : '';
				$return_msg['text'] .= ' in your cart';
