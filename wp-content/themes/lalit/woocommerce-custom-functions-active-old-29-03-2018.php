...
<?php	
	...
	add_action( 'woocommerce_register_post', 'the_lalit_validate_extra_register_fields', 10, 3 );
	function the_lalit_validate_extra_register_fields( $username, $email, $validation_errors ) 
	{
	...
	function the_lalit_validate_account_field($args)
	{
		...
		{
			if(isset($_POST['account_phone']) && !preg_match('/^[0-9]{10}+$/', trim($_POST['account_phone']))) 
			{
	...
	/*function the_lalit_after_add_to_cart_button() {
		...
		?>
		<div class="product-view-cart-container clearfix"<?php if($cart_count < 1){ ?> style="display:none;"<?php } ?>>
			<p class="view-cart-count-description">There <?php echo ($cart_count > 1) ? 'are' : 'is';?> <?php echo $cart_count; ?> item<?php echo ($cart_count > 1) ? 's' : '';?> in your cart</p>
