...
if ( ! empty( $breadcrumb ) ) {
	...
	foreach ( $breadcrumb as $key => $crumb ) {
		...
		if($breadcrumb_count == 0)
		{
			...
			$itemList[$i]['@type'] = 'ListItem';
			$itemList[$i]['position'] = $position + $i;
			$itemList[$i]['item']['@id'] = esc_url( $crumb[1] );
			$itemList[$i]['item']['name'] = 'Offers At The Lalit '.ucfirst($GLOBALS['location'][0]->slug);
		}
		else
		{
			if ( ! empty( $crumb[1] ) && sizeof( $breadcrumb ) !== $key + 1 ) 
			{
				...
				$itemList[$i]['position'] = $position + $i;
				$itemList[$i]['item']['@id'] = esc_url( $crumb[1] );
				$itemList[$i]['item']['name'] = esc_html( $crumb[0] );
			} 
			else 
			{
				...
				$itemList[$i]['@type'] = 'ListItem';
				$itemList[$i]['position'] = $position + $i;
				$itemList[$i]['item']['@id'] = get_the_permalink();
				$itemList[$i]['item']['name'] = esc_html( trim($p_name) );
			}
		}
