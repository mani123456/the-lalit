...
<form class="cart" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="group_table" width="100%">
		<tbody>
			<?php
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					?>
					<tr class="product-detail-row" id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
						<td class="label product-detail-label">
							<?php
							$product_display_name = get_post_meta( $grouped_product->get_id(), 'woocommerce_product_display_name', true );
							?>
							<label for="product-<?php echo $grouped_product->get_id(); ?>">
								<?php echo $grouped_product->is_visible() ? '<a class="product-detail-name" target="_blank" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $product_display_name . '</a>' : $product_display_name; ?>
							</label>
						</td>
						<?php do_action( 'woocommerce_grouped_product_list_before_price', $grouped_product ); ?>
						<td class="price product-detail-price">
							<?php
								echo $grouped_product->get_price_html();
								//echo wc_get_stock_html( $grouped_product );
							?>
						</td>
						<td class="product-detail-quantity">
	...
	<?php if ( $quantites_required ) : ?>
		...
		<div class="clearfix">
			<button type="submit" class="single_add_to_cart_button button alt product-detail-cart" disabled><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
		</div>
